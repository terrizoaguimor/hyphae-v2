...
\emph{tamper-evident}: a chain-aware adversary who rewrites the
persisted head defeats the bare chain, but is caught once the head
is externally anchored with an Ed25519 signature held outside the
store, which we implement and demonstrate --- so detection holds
against any attacker who does not also compromise the anchor key.
The sub-millisecond, CPU-only cost is a corollary of not
generating, shared with echo. We argue the durable contribution is the
